...
export class ViewService {
  ...
  constructor(
    private workpadState: WorkpadStateService,
    private fsState: FsStateService,
  ) {
  ...
  handleWorkpadEvents(event: NotepadEvents) {
    switch (event.type) {
      case AppEvents.WORKPAD_SAVE_REQUEST:
        ...
        if (
          this.fsState.currentTab()?.path !=
            this.workpadState.currentWorkpadFilePath() &&
          this.fsState.currentTab()?.title == 'New Tab'
        ) {
          this.fsState.currentTab.set({
            ...this.fsState.currentTab()!,
            title: this.workpadState.currentWorkingFileName()!,
            path: this.workpadState.currentWorkpadFilePath()!,
          });
          this.notepadEvents$.next({
            type: AppEvents.TAB_TITLE_CHANGE,
            file_name: this.workpadState.currentWorkingFileName(),
            path: this.workpadState.currentWorkpadFilePath(),
          });
        }
        break;
  ...
  setWorkpadConfigs(event: NotepadEvents, eventType: AppEvents, data: string) {
    this.workpadState.currentWorkbookContent$.next(data);
    this.workpadState.currentWorkingFileName.set(event.file_name);
    this.workpadState.currentWorkpadFilePath.set(event.path);
    this.notepadEvents$.next({
      ...event,
      type: eventType,
    });
  ...
  resetWorkpadConfig(event: NotepadEvents, eventType: AppEvents, data: string) {
    this.workpadState.currentWorkbookContent$.next(data);
    this.workpadState.currentWorkingFileName.set(undefined);
    this.workpadState.currentWorkpadFilePath.set(undefined);
    this.notepadEvents$.next({
      ...event,
      type: eventType,
    });
  ...
  saveFile(file: NotepadEvents): Promise<string> {
    ...
    } else if (file.data && this.workpadState.currentWorkpadFilePath()) {
